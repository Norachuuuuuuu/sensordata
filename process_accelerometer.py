        ...
        try:
            ...
            for test_type in test_types:
                ...
                if col_name in df.columns:
                    complete_count = df[col_name].notna().sum()
                    print(
                        f"  {test_type.replace('_', ' ').upper()}: {complete_count}/{len(df)} ({complete_count / len(df) * 100:.1f}%)")
            ...
            for col in rating_cols:
                if col in df.columns:
                    ...
                    if len(ratings) > 0:
                        print(
                            f"  {col.replace('_', ' ').title()}: Mean = {ratings.mean():.2f}, Median = {ratings.median():.2f}, Range = {ratings.min():.1f}-{ratings.max():.1f}")
            ...
            for col in rating_diff_cols:
                if col in df.columns:
                    ...
                    if len(diff) > 0:
                        print(
                            f"  {col.replace('_', ' ').title()}: Mean = {diff.mean():.2f}, Median = {diff.median():.2f}, Range = {diff.min():.1f}-{diff.max():.1f}")
            ...
            for metric_name, display_name in itug_metrics:
                ...
                if pre_col in df.columns and post_col in df.columns:
                    ...
                    if len(pre_data) > 0 and len(post_data) > 0:
                        ...
                        print(
                            f"    {display_name}: Pre = {pre_mean:.2f}, Post = {post_mean:.2f}, Change = {(post_mean - pre_mean):.2f} ({(post_mean - pre_mean) / pre_mean * 100:.1f}%)")
            ...
            for test_type in test_types:
                ...
                if pc1_mean_col in df.columns and pc1_std_col in df.columns:
                    ...
                    if len(pc1_mean) > 0 and len(pc1_std) > 0:
                        print(
                            f"  {test_type.replace('_', ' ').upper()}: Mean = {pc1_mean.mean():.3f}, StdDev = {pc1_std.mean():.3f}")
